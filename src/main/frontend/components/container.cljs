...
(rum/defc page-name-dot
  ...
  (let [original-name (db-model/get-page-original-name name)
        ...
        ctx-icon #(shui/tabler-icon %1 {:class "pr-1 opacity-80"})
          ...
          (shui/dropdown-menu-trigger
           (shui/button
            {:size     :sm
             ...
             :title    (t :page/unfavorite)
             :on-click (fn [e]
                         (page-handler/unfavorite-page! original-name)
                         (util/stop e))}
            [:i.relative {:style {:top "1px"}} (ctx-icon "star-off")])) ;; (shui/tabler-icon "dots") 
          ))]))))
...
(def help-menu-items
  ...
  ;;  :hr
   ...
   :hr
   {:title (t :help/changelog) :icon "asterisk" :href "https://github.com/YU000jp/fork-logseq/blob/develop/CHANGELOG.md"}])
